...
interface IPriceOracle {
    function peekPrice(address asset) external view returns (uint256 price, bool ok, uint256 numFresh, bool wouldTrip, bool paused);
}
...
contract CdpEngine is Supportable {
    ...
    function _price(address asset) internal view returns (uint256) {
        (uint256 px, bool ok, , bool wouldTrip, bool paused) = oracle.peekPrice(asset);
        require(ok && !wouldTrip && !paused, "oracle: unsafe price");
        return px;
    }

    function _ratioBps(uint256 collateral, uint256 debt) internal view returns (uint256) {
        if (debt == 0) return type(uint256).max;
        uint256 price = _price(address(collateralToken));
