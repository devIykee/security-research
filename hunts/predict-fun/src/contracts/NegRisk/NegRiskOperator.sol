...
contract NegRiskOperator is INegRiskOperatorEE, Auth {
    ...
    uint256 public immutable DELAY_PERIOD;
    ...
    /// @param _delayPeriod - the delay period for resolving questions
    constructor(address _nrAdapter, uint256 _delayPeriod) {
        nrAdapter = NegRiskAdapter(_nrAdapter);
        if (_delayPeriod > 1 hours) revert InvalidDelayPeriod();
        DELAY_PERIOD = _delayPeriod;
    }
    ...
    function prepareMarket(uint256 _feeBips, bytes calldata _data) external onlyOperator returns (bytes32) {
    ...
    function prepareCondition(bytes32, uint256) external {
        // no-op
    }
}
