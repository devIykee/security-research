...
import {YieldBearingWrappedCollateral} from "../YieldBearing/YieldBearingWrappedCollateral.sol";
import {MarketData, MarketStateManager} from "./modules/MarketDataManager.sol";
...
import {INegRiskAdapterEE} from "./NegRiskAdapter.sol";

/// @title YieldBearingNegRiskAdapter
...
contract YieldBearingNegRiskAdapter is ERC1155TokenReceiver, MarketStateManager, INegRiskAdapterEE, Auth {
    ...
    YieldBearingWrappedCollateral public immutable wcol;
    ...
    /**
     * @notice Mapping of user addresses to their permissions
     */
    mapping(address => Permissions permissions) public userPermissions;
    ...
    /**
     * @notice Enable or disable convert positions gating.
     *         It is disabled by default.
     *         If something breaks after enabling this, it can be disabled again
     *         to revert to the original behavior.
     */
    bool public isConvertPositionsGated = false;
    ...
    constructor(address _ctf, address _collateral, address _vault, address _yieldManager) {
        ctf = IConditionalTokens(_ctf);
        col = ERC20(_collateral);
        vault = _vault;

        wcol = new YieldBearingWrappedCollateral(_collateral, col.decimals(), _yieldManager);
     ...
     */
    function setIsSplitPositionGated(bool gated) external onlyAdmin {
        isSplitPositionGated = gated;
        emit IsSplitPositionGatedUpdated(gated);
    }
     ...
     */
    function setIsMergePositionsGated(bool gated) external onlyAdmin {
        isMergePositionsGated = gated;
        emit IsMergePositionsGatedUpdated(gated);
    }

    /**
     * @notice Enable or disable convert positions gating.
     * @param gated True to enable convert positions gating, false to disable it.
     */
    function setIsConvertPositionsGated(bool gated) external onlyAdmin {
        isConvertPositionsGated = gated;
        emit IsConvertPositionsGatedUpdated(gated);
    }

    /**
     * @notice Set the permissions for a user
     * @param _user The address of the user
     * @param _canSplitPosition True if the user can split positions
     * @param _canMergePositions True if the user can merge positions
     * @param _canConvertPositions True if the user can convert positions
     */
    function setPermissions(
        address _user,
        bool _canSplitPosition,
        bool _canMergePositions,
        bool _canConvertPositions
    ) external onlyAdmin {
        Permissions storage permissions = userPermissions[_user];
        permissions.canSplitPosition = _canSplitPosition;
        permissions.canMergePositions = _canMergePositions;
        permissions.canConvertPositions = _canConvertPositions;

        emit PermissionsUpdated(_user, _canSplitPosition, _canMergePositions, _canConvertPositions);
    }
    ...
    function splitPosition(bytes32 _conditionId, uint256 _amount) public {
        if (isSplitPositionGated && !userPermissions[msg.sender].canSplitPosition) revert Unauthorized();
    ...
    function mergePositions(bytes32 _conditionId, uint256 _amount) public {
        if (isMergePositionsGated && !userPermissions[msg.sender].canMergePositions) revert Unauthorized();
    ...
    function convertPositions(bytes32 _marketId, uint256 _indexSet, uint256 _amount) external {
        if (isConvertPositionsGated && !userPermissions[msg.sender].canConvertPositions) revert Unauthorized();
