...
contract BDexV2Locker is Ownable, ReentrancyGuard {
    ...
    /// @notice 永久锁仓使用的特殊解锁时间；该值对应 uint256 最大值。
    ...
    struct FeeStruct {
        ...
        IERCBurn secondaryFeeToken; // BDex or BDex
        uint256 secondaryTokenFee; // optional, BDex or BDex
     ...
     * @param _unlock_date 秒级 Unix 时间戳；传 `ETERNAL_LOCK` 表示永久锁定
     * @param _referral the referrer address if any or address(0) for none
     * @param _fee_in_eth fees can be paid in eth or in a secondary token such as BDex with a discount on univ2 tokens
    ...
    function lockLPToken(
        ...
        require(
            _unlock_date < 10000000000 || _unlock_date == ETERNAL_LOCK,
            "TIMESTAMP INVALID"
        ); // 普通锁防止误传毫秒时间戳，同时允许 uint256 最大值表示永久锁
        require(
            _unlock_date == ETERNAL_LOCK || _unlock_date > block.timestamp,
            "DATE PASSED"
        );
    ...
    /**
     * @notice extend a lock with a new unlock date, or convert it into an eternal lock
     */
    function relock(
        uint256 _lockID,
        uint256 _unlock_date
     ...
     * @dev 当 `_amount` 等于 `type(uint256).max` 时，自动取回该锁仓的全部剩余 LP。
     */
    function withdraw(uint256 _lockID, uint256 _amount) external nonReentrant {
        require(_amount > 0, "ZERO WITHDRAWL");
        TokenLock storage userLock = LOCKS[_lockID];
        require(userLock.owner == msg.sender, "NOT OWNER");
        require(userLock.unlockDate != ETERNAL_LOCK, "ETERNAL_LOCK");
        require(userLock.unlockDate < block.timestamp, "NOT YET");

        // 无需先读取精确余额即可发起“全部取回”；普通数值仍保留部分取回语义。
        uint256 withdrawAmount = _amount == type(uint256).max
            ? userLock.amount
            : _amount;
        require(withdrawAmount > 0, "ZERO WITHDRAWL");
        userLock.amount -= withdrawAmount;

        // clean user storage
        if (userLock.amount == 0) {
            EnumerableSet.UintSet storage userLocks = USERS[msg.sender]
                .locksForToken[userLock.lpToken];
    ...
    function splitLock(
        ...
        EnumerableSet.UintSet storage user_locks = user.locksForToken[
            userLock.lpToken
        ];
