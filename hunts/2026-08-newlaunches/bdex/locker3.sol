...
} from "./uniswap-updated/INonfungiblePositionManager.sol";
import {ICountryList} from "./ICountryList.sol";
import {IMigrateV3NFT} from "./IMigrateV3NFT.sol";
import {IBDexV3LiquidityLocker} from "./IBDexV3LiquidityLocker.sol";

/*
    Version 2.0

    The BDex UniV3 Liquidity Locker Contract locks whitelisted exact forks of Uniswap V3's liquidity NFT's in this contract.
...
interface IFeeResolver {
    function useFee(
        bytes[] memory r,
        address sender
    ) external returns (IBDexV3LiquidityLocker.FeeStruct memory fee);
}

contract BDexV3LiquidityLocker is
    IBDexV3LiquidityLocker,
    Ownable,
    IERC721Receiver,
    ReentrancyGuard
{
    ...
    address public AUTO_COLLECT_ACCOUNT; // account controlled by BDex to auto collect fees if a fee option involving collection fees was accepted
     ...
     * @param params The locking params as seen in IBDexV3LiquidityLocker.sol
     *
    ...
    /**
     * @notice 批量模拟 collect，并通过自定义错误返回结果。
     * @dev 前端必须使用 eth_call 并解析 CollectBatchPreviewResult；
     *      最终 revert 会原子回滚 Position Manager、ERC20 和 Locker 的全部状态变化。
     */
    function collectBatchPreview(
        ...
        if (
            lockIds.length != recipients.length ||
            lockIds.length != amount0Maxs.length ||
            lockIds.length != amount1Maxs.length
        ) {
            revert LengthMismatch();
        }

        CollectPreviewItem[] memory previews = new CollectPreviewItem[](
            lockIds.length
        );
        for (uint256 i; i < lockIds.length; ) {
            previews[i] = _collectPreviewItem(
                lockIds[i],
                recipients[i],
                amount0Maxs[i],
                amount1Maxs[i]
        ...
        revert CollectBatchPreviewResult(previews);
    }

    function _collectPreviewItem(
        uint256 lockId,
        address recipient,
        uint128 amount0Max,
        uint128 amount1Max
    ) private returns (CollectPreviewItem memory preview) {
        Lock memory userLock = LOCKS[lockId];
        preview.lockId = lockId;
        preview.recipient = recipient;
        (, , preview.token0, preview.token1, , , , , , , , ) = userLock
            .nftPositionManager
        ...
        (
            ...
            preview.fee0,
            preview.fee1
        ) = _collect(lockId, recipient, amount0Max, amount1Max);
    }
     ...
     * This method is here just for convenience for some contracts which solely interact with the BDex lockers / lockIds
     */
