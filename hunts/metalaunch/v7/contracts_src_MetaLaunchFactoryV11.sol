...
import {MetaLaunchTokenDeployerV11} from "./MetaLaunchTokenDeployerV11.sol";
...
/// @title  MetaLaunchFactoryV11
/// @notice V11 launchpad for Robinhood Chain. Owner-managed DEX registry + launch
///         templates; deploys a fixed-supply ownerless MetaLaunchTokenV11 via
///         CREATE2 to a `…c0de` address, opens its Uniswap v3 pool at a configured
///         tick, deposits ~100% of supply as ONE-SIDED concentrated liquidity,
///         permanently locks the position NFT in MetaLaunchLockerV11, and (opt.)
///         executes a slippage-protected, allocation-capped creator initial buy —
///         all atomically. No bonding-curve contract, no migration: the token is
///         on Uniswap from block one. "Graduation" is a read-only paired-principal
///         milestone derived from the locked V3 position.
///
///         This factory NEVER alters V10 tokens or positions.
contract MetaLaunchFactoryV11 is Ownable2Step, ReentrancyGuard {
    ...
    uint16 public constant MIN_LIMIT_BPS = 10; // 0.1% floor on wallet/tx caps
    ...
    struct TokenParams {
        ...
        string logo;
        string description;
        MetaLaunchTokenV11.Socials socials;
        address feeWallet;
    }
    ...
    MetaLaunchTokenDeployerV11 public immutable tokenDeployer;
        ...
        Ownable(owner_)
    {
        ...
        tokenDeployer = MetaLaunchTokenDeployerV11(tokenDeployer_);
    ...
    function _validateLaunchConfig(LaunchConfig calldata c) internal pure {
        ...
        if (c.maxWalletBps < MIN_LIMIT_BPS || c.maxWalletBps > BPS) revert InvalidWalletLimit();
        if (c.maxTxBps < MIN_LIMIT_BPS || c.maxTxBps > BPS) revert InvalidTxLimit();
        ...
        if (c.restrictionBlocks > MAX_RESTRICTION_BLOCKS) revert InvalidRestriction();
    ...
    function _deployVanityToken(
        ...
        MetaLaunchTokenV11.InitParams memory ip = _initParams(params, cfg, dex);
    ...
    function _initParams(TokenParams calldata params, LaunchConfig memory cfg, DexConfig memory dex)
        internal
        view
        returns (MetaLaunchTokenV11.InitParams memory ip)
    {
        ip = MetaLaunchTokenV11.InitParams({
            name: params.name,
            ...
            maxTxBps: cfg.maxTxBps,
            restrictionBlocks: cfg.restrictionBlocks
        });
    }
    ...
    ///         the exact wallet that will launch.
    ...
    function _openPoolAndLock(
        ...
        _launched[x.token] = LaunchedToken({
            ...
            restrictionsEndBlock: block.number + x.cfg.restrictionBlocks,
            supply: x.cfg.supply,
    ...
    ///      router, delivered to the fee wallet, with slippage + allocation caps.
    function _initialBuy(
        ...
        // open the token's one-shot launch-block exemption for the buyer
        MetaLaunchTokenV11(x.token).setInitialBuyRecipient(recipient);
        ...
        // 23. clear the exemption immediately
        MetaLaunchTokenV11(x.token).setInitialBuyRecipient(address(0));
