...
/// @title  MetaLaunchTokenDeployerV12
/// @notice Thin, ownerless CREATE2 deployer that holds the MetaLaunchTokenV12
...
///         the token creation code itself.
contract MetaLaunchTokenDeployerV12 {
    ...
    /// @notice Deploy a MetaLaunchTokenV12 via CREATE2. Factory-only.
    function deploy(bytes32 salt, MetaLaunchTokenV12.InitParams calldata ip) external returns (address token) {
        if (msg.sender != factory) revert NotFactory();
        token = address(new MetaLaunchTokenV12{salt: salt}(ip));
    }
    ...
    function tokenInitCodeHash(MetaLaunchTokenV12.InitParams calldata ip) external pure returns (bytes32) {
        return keccak256(abi.encodePacked(type(MetaLaunchTokenV12).creationCode, abi.encode(ip)));
    }
