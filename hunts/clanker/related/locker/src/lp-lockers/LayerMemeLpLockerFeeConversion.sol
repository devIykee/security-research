...
import {ILayerMeme} from "../interfaces/ILayerMeme.sol";
import {ILayerMemeFeeLocker} from "../interfaces/ILayerMemeFeeLocker.sol";

import {ILayerMemeHook} from "../interfaces/ILayerMemeHook.sol";
import {ILayerMemeLpLocker} from "../interfaces/ILayerMemeLpLocker.sol";
import {ILayerMemeLpLockerFeeConversion} from "./interfaces/ILayerMemeLpLockerFeeConversion.sol";
...
contract LayerMemeLpLockerFeeConversion is ILayerMemeLpLockerFeeConversion, ReentrancyGuard, Ownable {
    ...
    ILayerMemeFeeLocker public immutable feeLocker;
    ...
    constructor(
        address owner_,
        address factory_, // Address of the layerMeme factory
        address feeLocker_,
        ...
        factory = factory_;
        feeLocker = ILayerMemeFeeLocker(feeLocker_);
    ...
    function placeLiquidity(
        ILayerMeme.LockerConfig memory lockerConfig,
        ILayerMeme.PoolConfig memory poolConfig,
        ...
        FeeIn[] memory lpFeeConversionPreferences = abi.decode(
            lockerConfig.lockerData, (ILayerMemeLpLockerFeeConversion.LpFeeConversionInfo)
        ).feePreference;
    ...
    function _mintLiquidity(
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            if (lockerConfig.tickLower[i] < poolConfig.tickIfToken0IsLayerMeme) {
        ...
        bool token0IsLayerMeme = token < poolConfig.pairedToken;
        ...
        int24 startingTick =
            token0IsLayerMeme ? poolConfig.tickIfToken0IsLayerMeme : -poolConfig.tickIfToken0IsLayerMeme;
        ...
        for (uint256 i = 0; i < lockerConfig.tickLower.length; i++) {
            ...
            uint256 amount0 = token0IsLayerMeme ? tokenAmount : 0;
            uint256 amount1 = token0IsLayerMeme ? 0 : tokenAmount;

            // determine tick bounds for this position
            int24 tickLower_ =
                token0IsLayerMeme ? lockerConfig.tickLower[i] : -lockerConfig.tickLower[i];
            int24 tickUpper_ =
                token0IsLayerMeme ? lockerConfig.tickUpper[i] : -lockerConfig.tickUpper[i];
            int24 tickLower = token0IsLayerMeme ? tickLower_ : tickUpper_;
            int24 tickUpper = token0IsLayerMeme ? tickUpper_ : tickLower_;
    ...
    function _mevModuleOperating(address token) internal view returns (bool) {
        ...
        if (!ILayerMemeHook(address(_tokenRewards[token].poolKey.hooks)).mevModuleEnabled(poolId)) {
            return false;
        }
        ...
        uint256 poolCreationTimestamp =
            ILayerMemeHook(address(_tokenRewards[token].poolKey.hooks)).poolCreationTimestamp(poolId);
        if (
            poolCreationTimestamp
                + ILayerMemeHook(address(_tokenRewards[token].poolKey.hooks)).MAX_MEV_MODULE_DELAY()
                <= block.timestamp
        ) {
            return false;
        }
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        // if the rewardToken is the layerMeme, we want to swap for recipients who
        // want their fee in the paired token
        //
        // conversely, if the rewardToken is the paired token, we want to swap for
        // recipients who want their fee in the layerMeme token
        FeeIn toSwap = token == rewardToken ? FeeIn.Paired : FeeIn.LayerMeme;
    ...
    function onERC721Received(address, address from, uint256 id, bytes calldata)
        ...
        returns (bytes4)
    {
        // Only LayerMeme Factory can send NFTs here
        if (from != factory) {
            revert Unauthorized();
        }
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return interfaceId == type(IERC721Receiver).interfaceId
            || interfaceId == type(ILayerMemeLpLocker).interfaceId;
    }
}