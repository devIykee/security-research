...
interface INonfungiblePositionManager {
    ...
    /// @notice Collects up to `amount{0,1}Max` of accrued swap fees for a position, sent to `recipient`
    function collect(CollectParams calldata params)
        external
        payable
        returns (uint256 amount0, uint256 amount1);

    /// @notice Returns the full on-chain state of a position (token0/token1 among others)
