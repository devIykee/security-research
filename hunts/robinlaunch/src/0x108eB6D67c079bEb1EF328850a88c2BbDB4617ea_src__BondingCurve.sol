 ...
 *       – Sends the LP NFT to `lpFeeCollector` (if set) so post-graduation swap fees
 *         stay collectible and split 50/50 creator/protocol — falls back to the burn
 *         address if no collector was configured. Either way liquidity is locked forever;
 *         see LpFeeCollector.sol for the no-rug guarantee.
...
contract BondingCurve is ReentrancyGuard {
    ...
    /// LP NFT is sent here when no lpFeeCollector is configured — locked forever, but
    /// post-graduation swap fees become permanently unreachable (see LpFeeCollector.sol).
    ...
    /// Recipient of the graduation LP NFT. address(0) preserves the legacy
    /// "burn to LOCK_ADDRESS" behaviour for factories deployed without one.
    ...
    function _graduate() internal {
        ...
        // 6. Mint full-range LP position — recipient is the LpFeeCollector (if configured),
        //    so post-graduation swap fees stay collectible; otherwise falls back to 0xdead.
        ...
        // 7. Register the position's creator in the same transaction as the mint,
        //    so there is never a window where the NFT exists without a registered creator.
