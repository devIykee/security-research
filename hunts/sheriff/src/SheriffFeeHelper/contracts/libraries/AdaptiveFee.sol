...
library AdaptiveFee {
    ...
    function initialFeeConfiguration() internal pure returns (AlgebraFeeConfiguration memory) {
        return AlgebraFeeConfiguration({
            alpha1: 3000 - INITIAL_MIN_FEE, // max value of the first sigmoid in hundredths of a bip
            alpha2: 15000 - 3000, // max value of the second sigmoid in hundredths of a bip
            ...
            baseFee: INITIAL_MIN_FEE // in hundredths of a bip
        });
    }
    ...
    function validateFeeConfiguration(AlgebraFeeConfiguration memory _config) internal pure {
        require(
            uint256(_config.alpha1) + uint256(_config.alpha2) + uint256(_config.baseFee)
                <= type(uint16).max,
            "Max fee exceeded"
        );
    ...
    function getFee(uint88 volatility, AlgebraFeeConfigurationU144 config)
        internal
        pure
        returns (uint16 fee)
    {
    ...
    function sigmoid(uint256 x, uint16 g, uint16 alpha, uint256 beta)
        internal
        pure
        returns (uint256 res)
    {
    ...
    function expXg4(uint256 x, uint16 g, uint256 gHighestDegree)
        internal
        pure
        returns (uint256 res)
    {
        ...
        assembly {
            let xdg := div(x, g)
            switch xdg
            case 0 { closestValue := 100000000000000000000 } // 1
            case 1 { closestValue := 271828182845904523536 } // ~= e
            case 2 { closestValue := 738905609893065022723 } // ~= e^2
            case 3 { closestValue := 2008553692318766774092 } // ~= e^3
            case 4 { closestValue := 5459815003314423907811 } // ~= e^4
            default { closestValue := 14841315910257660342111 } // ~= e^5
        ...
        unchecked {
            ...
            // After calculating the closestValue x/g is <= 0.5, so that the series in the
            // neighborhood of zero converges with sufficient speed
